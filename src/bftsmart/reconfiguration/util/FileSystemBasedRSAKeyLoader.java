...
//			+ "\nAKCTm3tbbwLJnnPN46mAgrYb5+LFOmHyNtEDgjxEVrzpQaCChZici2YGY1jTBjb/De4jii8RXllA\n" + "\ntUhBEsqyXDA=";
	...
	private DefaultRSAKeyLoader defRSAKeyLoader;
	...
	public PublicKey loadPublicKey(int id) throws Exception {
		...
		if (defaultKeys) {
			return defRSAKeyLoader.loadPublicKey(id);
		}
		...
		f.close();
		r.close();
		PublicKey ret = RSAKeyUtils.getPublicKeyFromString(key);
		return ret;
	}

	// /* (non-Javadoc)
	// * @see bftsmart.reconfiguration.util.KeyLoader#loadPublicKey()
	// */
	// @Override
	...
	public PrivateKey loadPrivateKey(int id) throws Exception {
		...
		if (defaultKeys) {
			return defRSAKeyLoader.loadPrivateKey(id);
		}

		if (priKey == null) {
			FileReader f = new FileReader(path + "privatekey" + id);
			BufferedReader r = new BufferedReader(f);
			String tmp = "";
			String key = "";
			while ((tmp = r.readLine()) != null) {
			...
			f.close();
			r.close();
			priKey = RSAKeyUtils.getPrivateKeyFromString(key);
		}
		return priKey;
	}
